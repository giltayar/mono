...
test('add and remove products for sale dynamically', async ({page}) => {
  ...
  await newForm.productsForSale().productInput(3).locator.blur()
  await page.waitForLoadState('networkidle')
...
test('product link navigates to product page and updates when product changes', async ({page}) => {
  await page.goto(new URL('/sales-events', url()).href)
  ...
  const newForm = newSalesEventModel.form()
  await newForm.nameInput().locator.fill('Link Test Event')
  ...
  // Click the product link and verify it navigates to the product page
  await updateForm.productsForSale().productLink(0).locator.click()
  await expect(updateProductModel.pageTitle().locator).toHaveText('Update Product 1')

  await page.goBack()

  // Change the product and verify the link updates
  await updateForm.productsForSale().productInput(0).locator.fill('2')
  await updateForm.productsForSale().productInput(0).locator.blur()
  await page.waitForLoadState('networkidle')

  await updateForm.productsForSale().productLink(0).locator.click()
  await expect(updateProductModel.pageTitle().locator).toHaveText('Update Product 2')
})
