...
const {url, sql, cardcomIntegration, smooveIntegration, ravmesserIntegration, academyIntegration} =
...
for (const provider of mailingListProviders) {
  test(`disconnect cardcom sale removes disconnect button and disconnects from external providers (${provider})`, async ({
    page,
  }) => {
    ...
    const productNumber = await createProduct(
      {
        ...
        ...mailingListProductFields(provider, {listId: mainListId, removedListId: 3}),
        academyCourses: [
    ...
    await expect(
      academyCourses.courseCheckbox(`carmel/${academyCourseId}`).locator,
    ).not.toBeChecked()
    await expect(
      academyCourses.courseCheckbox(`inspiredlivingdaily/${ildAcademyCourseId}`).locator,
    ).not.toBeChecked()

    // Main list should be unchecked, removed list should be checked
    const mailingLists = mailingListCard(productCard, provider)
    await expect(mailingLists.mainListCheckbox().locator).not.toBeChecked()
    await expect(mailingLists.removedListCheckbox().locator).toBeChecked()
  ...
  test(`disconnect manual sale removes disconnect button and disconnects from external providers (${provider})`, async ({
    page,
  }) => {
    const academyCourseId = 1
    const ildAcademyCourseId = 100
    const mainListId = 2
    const removedListId = 3
    ...
    const productNumber = await createProduct(
      {
        name: 'Test Product',
        productType: 'recorded',
        ...mailingListProductFields(provider, {listId: mainListId, removedListId}),
        academyCourses: [
          {courseId: academyCourseId, accountSubdomain: 'carmel'},
          {courseId: ildAcademyCourseId, accountSubdomain: 'inspiredlivingdaily'},
        ],
    ...
    // Main list should be checked again
    const mailingLists2 = mailingListCard(productCard2, provider)
    await expect(mailingLists2.mainListCheckbox().locator).toBeChecked()
